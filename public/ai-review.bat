@echo off
setlocal

:: -------------------------------------------------------------------
::  B) Define output filenames
:: -------------------------------------------------------------------
set "OUTPUT_FILE=consolidated.txt"
set "FILELIST=filelist.txt"

:: Clear the output file
> "%OUTPUT_FILE%" echo.

:: -------------------------------------------------------------------
::  C) Generate the file list with correct paths
:: -------------------------------------------------------------------
> "%FILELIST%" (

    echo .htaccess
    rem echo tables.txt
    echo public\js\c64Syntax.js  
    echo public\js\cartManager.js  
    echo public\js\commandManager.js
    echo public\js\emulatorManager.js
    echo public\js\gameManager.js
    echo public\js\productServices.js
    echo public\js\snipcartKeys.js
    echo public\js\uiManager.js
    echo public\js\userManager.js
    echo public\js\googleauth.js
    rem echo public\js\ViewRenderer.js
    rem echo public\js\displayManager.js
    echo public\emulator\jcs64.html
    echo public\auth.php
    echo public\user_api.php
    echo public\logout.php
    echo public\login.php
    echo public\authCallback.php
    echo public\google_auth_handler.php
    echo public\c64\tictactoe.html


    echo directus-docker\docker-compose.yml

    echo public\admin.yaml

    
    
    rem echo public\js\userManager.js
    rem echo public\Token.txt
    echo public\collections_api.php
    echo config_directus.php
    echo public\config.php
    echo public\directus_proxy.php
    echo public\config.php
    echo public\index.html
    echo public\login.php
    echo public\logout.php
    echo public\products_api.php
    echo public\snipcart_webhook.php
    echo public\css\style.css
    echo public\user_api.php

)

:: Process each file
for /f "usebackq delims=" %%F in ("%FILELIST%") do (
    if exist "%%F" (
        (
            echo ==============================
            echo Path: %%F
            echo ==============================
            type "%%F"
            echo.
            echo.
        ) >> "%OUTPUT_FILE%"
    ) else (
        echo File not found: %%F >> "%OUTPUT_FILE%"
    )
)

:: Clean up and finish
del "%FILELIST%"
echo Consolidation complete. The result is saved in "%OUTPUT_FILE%"
pause
endlocal